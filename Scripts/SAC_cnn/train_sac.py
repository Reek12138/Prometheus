...
network = SACContinuous(state_dim=robot_dim, hidden_dim=512, action_dim=action_dim,
                        actor_lr=1e-4, critic_lr=1e-4, alpha_lr=1e-4,
                        target_entropy=-math.log(2), tau=tau, gamma=discount, device=device, cnn_output_dim=20)
...
def evaluate(eval_ep):
    ...
    while eval_episode_i <= eval_ep:
        ...
        eval_laser_array, eval_state = env.reset()
        time.sleep(0.5)
        eval_done = False
        ...
        while eval_step_i <= num_eval_step:
            ...
            eval_next_laser_array,eval_next_state, eval_reward, eval_done, eval_target = env.step(eval_action_in)

            eval_state = eval_next_state
            eval_laser_array = eval_next_laser_array
...
while episode_i <= num_episode:
    ...
    while step_i <= num_step:
        ...
        action = network.get_action(state,laser_array)
        ...
        next_laser_array, next_state, reward, done, target = env.step(action_in)
        done_bool = step_i + 1 == num_step or done
        replay_buffer.add(state, action, reward, done_bool, next_state, laser_array, next_laser_array)
        # if step_i % 2 == 0:
        #     print(f"action_in: [{action_in[0]:.2f}, {action_in[1]:.2f}], reward: {reward:.4f}, state: {state}")

        state = next_state
        laser_array = next_laser_array

        if (total_step + 1) % train_frequence == 0 and total_step > batch_size:
            iter_count += 1
            s, a, r, d, ns, l, nl = replay_buffer.sample_batch(batch_size)
            transition_dict = {'states': s, 'actions': a, 'rewards': r, 'next_states': ns, 'dones': d, 'laser_array':l, 'next_laser_array':nl}
