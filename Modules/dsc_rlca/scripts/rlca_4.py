...
def main():
    ...
    # 确保所有参数都已设置
    params = [
    '/RLCA_node_4/swarm_num_uav', 
    '/RLCA_node_4/self_id', 
    '/RLCA_node_4/start_point_x', 
    '/RLCA_node_4/start_point_y', 
    '/RLCA_node_4/end_point_x', 
    '/RLCA_node_4/end_point_y'
    ]
    ...
    uav_num = int(rospy.get_param('/RLCA_node_4/swarm_num_uav', 4))
    uav_id = int(rospy.get_param('/RLCA_node_4/self_id', '999'))
    self_id = uav_id -1
    vel = 1.0

    start_point_x = float(rospy.get_param('/RLCA_node_4/start_point_x'))
    rospy.loginfo(f"UAV{uav_id} Parameter start_point_x: {start_point_x}")

    start_point_y = float(rospy.get_param('/RLCA_node_4/start_point_y'))
    rospy.loginfo(f"UAV{uav_id} Parameter start_point_y: {start_point_y}")

    end_point_x = float(rospy.get_param('/RLCA_node_4/end_point_x'))
    rospy.loginfo(f"UAV{uav_id} Parameter end_point_x: {end_point_x}")

    end_point_y = float(rospy.get_param('/RLCA_node_4/end_point_y'))
