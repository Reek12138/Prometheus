...
void NMPC::set_my_nmpc_solver()
{
    ...
    casadi::SX x = casadi::SX::sym("x");
    casadi::SX y = casadi::SX::sym("y");
    // casadi::SX yaw = casadi::SX::sym("yaw");

    casadi::SX vx = casadi::SX::sym("vx");
    casadi::SX vy = casadi::SX::sym("vy");
    // casadi::SX vyaw = casadi::SX::sym("vyaw"); // 偏航角的变化率或直接控制值

    // casadi::SX states = casadi::SX::vertcat({x, y, yaw});
    // casadi::SX controls = casadi::SX::vertcat({vx, vy, vyaw});
    casadi::SX states = casadi::SX::vertcat({x, y});
    casadi::SX controls = casadi::SX::vertcat({vx, vy});
    ...
    //根据上述模型函数向前预测无人车运动状态
    X(casadi::Slice(),0) = opt_para(casadi::Slice(0,2,1)); //状态初始值
    ...
    casadi::SX m_Q = casadi::SX::zeros(2,2);
    casadi::SX m_R = casadi::SX::zeros(2,2);
    // m_Q(0,0) = 0.5;
    // m_Q(1,1) = 0.5;
    // double y_min = 0.5;
...
void NMPC::opti_solution(Eigen::Matrix<float,2,1> current_states)
{
    ...
    m_args["p"] = parameters;
    m_args["lbg"] = std::vector<float>{-0.05, -0.05}; // 终端约束的下界
    m_args["ubg"] = std::vector<float>{0.05, 0.05}; // 终端约束的上界
    //求解
    ...
    std::vector<float>  res_control_vx, res_control_vy,res_control_vyaw;
    ...
    {
        initial_guess.push_back(res_control_vx.at(j));
    }
    initial_guess.push_back(res_control_vx.at(m_predict_step-1));
    // vy
    for(int j=0;j<m_predict_step-1;j++)
    {
    ...
    std::vector<Eigen::Matrix<float,2,1>> predict_X;
    ...
    for(int j=0; j<m_predict_step; j++)
    {
        Eigen::Matrix<float,2,1> Next_X;
        // 使用vx和vy直接更新x和y的位置，使用vyaw更新theta（偏航角）
        Next_X << predict_X.at(j)[0] + m_sample_time * res_control_vx.at(j),  // X方向
                predict_X.at(j)[1] + m_sample_time * res_control_vy.at(j);// Y方向
                // predict_X.at(j)[2] + m_sample_time * res_control_vyaw.at(j); // 偏航角
        predict_X.push_back(Next_X);
    }
...
std::vector<Eigen::Matrix<float,2,1>> NMPC::get_predict_trajectory()
{
