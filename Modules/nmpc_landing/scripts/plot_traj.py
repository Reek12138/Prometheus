...
# 替换为你的rosbag文件路径
bag_file = '/home/reek/Prometheus/Modules/nmpc_landing/rosbag/2024-09-17-20-41-34.bag'
...
with rosbag.Bag(bag_file, 'r') as bag:
    ...
    for topic, msg, t in bag.read_messages(topics=['/landing_pad_trajectory', 
                                                   '/prometheus/drone_trajectory']):
        ...
        if relative_time <= time_threshold:
            ...
            if topic == '/landing_pad_trajectory':
            ...
            elif topic == '/prometheus/drone_trajectory':
                for pose_stamped in msg.poses:
...
# 设置X, Y, Z轴的刻度间隔
ax.set_xticks(np.arange(-11, 11, 2))  # X轴从0到10，间隔为2
ax.set_yticks(np.arange(-5, 1, 1))  # Y轴从-20到0，间隔为5
...
print("Trajectory plot saved to {}".format(pdf_path))
