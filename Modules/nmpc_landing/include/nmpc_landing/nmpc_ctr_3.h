...
private:
    ...
    int n_states; //共有n个状态-----2
    int n_controls; //共有n个控制输入-----2
    std::vector<Eigen::Matrix<float,2,1>> m_predict_trajectory; //预测轨迹 
    std::vector<float> m_initial_guess; //初始猜测解
    Eigen::Matrix<float,2,1> m_goal_states, m_states, m_vels, m_goal_vels;
    //符号定义
...
public:
    ...
    //获取目标点
    void set_goal_states(Eigen::Matrix<float,2,1> goal_states, Eigen::Matrix<float,2,1> current_states, Eigen::Matrix<float,2,1> current_vels, Eigen::Matrix<float,2,1> goal_vels);
    //优化求解
    void opti_solution(Eigen::Matrix<float,2,1> current_states);
    //获取最优控制向量
    Eigen::Matrix<float,2,1> get_controls();
    //获取预测轨迹
    std::vector<Eigen::Matrix<float,2,1>> get_predict_trajectory();
