...
int main(int argc, char **argv)
{
    ...
    uav_pubs[3] = n.advertise<prometheus_msgs::SwarmCommand>("/uav4/prometheus/swarm_command", 1000);
...
void rvo_velCallback(const gazebo_msgs::ModelStates::ConstPtr &sub_msg)
{
    ...
    for (int i = 0; i < total_num; i++)
    {
        ...
        std::string agent_name = "uav" + std::to_string(i + 1);
        ...
        if (iter_agent != models_name.end())
        {
            ...
            Command_Now[i].header.stamp = ros::Time::now();
            Command_Now[i].header.frame_id = "world";
            Command_Now[i].source = "rvo";
            Command_Now[i].Command_ID = seq;
            Command_Now[i].Mode = prometheus_msgs::SwarmCommand::Takeoff;
            Command_Now[i].Move_mode = prometheus_msgs::SwarmCommand::XY_VEL_Z_POS;
            Command_Now[i].position_ref = {0.0, 0.0, 2.0};  // Target position (x, y, z)
            Command_Now[i].velocity_ref = {x, y, 0.0};  // Target velocity
            Command_Now[i].acceleration_ref = {0.0, 0.0, 0.0};  // Target acceleration
            Command_Now[i].yaw_ref = 0.0;  // Target yaw angle
            Command_Now[i].yaw_rate_ref = 0.0;  // Target yaw rate

            count_vel++;
            std::cout << "Current " << agent_name << std::endl;
            uav_pubs[i].publish(Command_Now[i]);
        }
    }
